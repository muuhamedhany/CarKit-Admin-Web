  ...
  return (
    <div className="space-y-6">
      <div className="sm:flex sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight" style={{ color: '#FFFFFF' }}>Vendors</h1>
          <p className="mt-1 text-sm" style={{ color: '#9E9E9E' }}>
            Review vendor applications and documents.
            {!loading && <span style={{ color: '#E91E8C' }}> ({vendors.length} total)</span>}
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl p-12 text-center" style={{ background: '#12121F', border: '1px solid #2A2A3A' }}>
          <Store className="w-12 h-12 mx-auto mb-3" style={{ color: '#6B6B80' }} />
          <p style={{ color: '#6B6B80' }}>No vendors found.</p>
        </div>
      ) : (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-sm" style={{ color: '#FFFFFF' }}>{vendor.name}</p>
                      <p className="text-xs mt-0.5" style={{ color: '#6B6B80' }}>{vendor.contact_info || 'No contact info'}</p>
                    </div>
                  </div>
                  <span
